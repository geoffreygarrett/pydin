...
public:
    ...
    void set_max_allowed_parallelism(std::size_t num_threads) {
        control = std::make_unique<tbb::global_control>(tbb::global_control::max_allowed_parallelism, num_threads);
    }
...
void bind_tbb(py::module &m) {
    py::class_<TBBControl>(m, "TBBControl")
            ...
            .def_property("max_allowed_parallelism", &TBBControl::get_max_allowed_parallelism,
                          &TBBControl::set_max_allowed_parallelism,
                          "Get or set the maximum number of threads TBB is allowed to utilize.")
            .def("get_thread_stack_size", &TBBControl::get_thread_stack_size,
                 "Get the stack size of the threads TBB creates.")
            .def("hardware_concurrency", &TBBControl::hardware_concurrency,
