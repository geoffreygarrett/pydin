...
int executeAddFunc() {
    ...
    std::unique_ptr<llvm::Module>      TheModule
            = std::make_unique<llvm::Module>("my cool jit", *TheContext);
    llvm::IRBuilder<> Builder(*TheContext);

    llvm::FunctionType *funcType = llvm::FunctionType::get(Builder.getInt32Ty(), false);
    llvm::Function     *addFunc  = llvm::Function::Create(funcType,
                                                     llvm::Function::ExternalLinkage,
                                                     "add",
                                                     &*TheModule);
    llvm::BasicBlock   *BB       = llvm::BasicBlock::Create(*TheContext, "entry", addFunc);
    Builder.SetInsertPoint(BB);
    llvm::Value *result = Builder.CreateAdd(Builder.getInt32(2), Builder.getInt32(3), "add");
    ...
    llvm::Function *F = TheModule->getFunction("add");
    if (!F) {
        llvm::errs() << "Unknown function referenced\n";
        return -1;
    }

    void *FPtr      = TheExecutionEngine->getPointerToFunction(F);
    int (*Result)() = (int (*)())(intptr_t) FPtr;
    return Result();// Execute the function
}
...
void bind_odinscript(py::module &m) {
    m.def("jit", &jit);
    m.def("executeAddFunc", &executeAddFunc, "A function which creates an LLVM function that adds two numbers and then executes it");
